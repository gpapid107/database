...
CREATE TABLE IF NOT EXISTS `mydb`.`Medical_Action` (
  ...
  `Action_Duration` INT NULL,     -- in minutes
...
CREATE TABLE IF NOT EXISTS `mydb`.`Prescription` (
  ...
  CONSTRAINT `unique_prescription_combo` UNIQUE (`Doctor_AMKA`, `Patient_AMKA`, `EMA_Code`, `Start_Date`)
)ENGINE = InnoDB;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 
        FROM Prescription
        JOIN Admission ON Prescription.Patient_AMKA = Admission.Patient_AMKA
        WHERE Admission.AdmissionID = NEW.AdmissionID
...
DELIMITER //
CREATE TRIGGER `chk_max_shifts_per_month` BEFORE INSERT ON `Shift_Staff` FOR EACH ROW
BEGIN
    ...
    IF v_staff_type = 'Ιατρός' AND v_shift_count >= 15 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Υπέρβαση ορίου. Οι ιατροί επιτρέπεται να κάνουν έως 15 βάρδιες τον μήνα.';
    ELSEIF v_staff_type = 'Νοσηλευτής' AND v_shift_count >= 20 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Υπέρβαση ορίου. Οι νοσηλευτές επιτρέπεται να κάνουν έως 20 βάρδιες τον μήνα.';
    ELSEIF v_staff_type = 'Διοικητικός' AND v_shift_count >= 25 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Υπέρβαση ορίου. Το διοικητικό προσωπικό επιτρέπεται να κάνει έως 25 βάρδιες τον μήνα.';
    END IF;
END //
DELIMITER ;
